...
## Phase 6: URL解析とレポート生成 ← **次のメインフェーズ**
- [ ] Netlify Functions の analyze API を作る（`netlify/functions/analyze.ts`）
- [ ] URLの本文を取得する処理を作る（`lib/websiteParser.ts`）
- [ ] URL要約処理を作る（LLM呼び出し）
- [ ] 自由記述の要約処理を作る
- [ ] 診断結果の自然文レポートをLLMで生成する（現状はルールベース固定文）
- [ ] `netlify.toml` を作成する

## Phase 6b: 結果画面UI強化・PDF対応 ← **2026-04-09 着手**
...
## Phase 7: 公開準備
- [ ] Netlify 用の設定を確認する
- [ ] 環境変数を設定する（`ANTHROPIC_API_KEY` または `OPENAI_API_KEY`）
